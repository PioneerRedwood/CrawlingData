...
brands = []
with open('2020_TotalPageDetail.csv', 'r', encoding='utf-8-sig') as f:
    reader = csv.reader(f)
    i = 0
    for row in reader:
        if i == 0:
...
with open('2019_TotalPageDetail.csv', 'r', encoding='utf-8-sig') as f:
    reader = csv.reader(f)
    i = 0
    for row in reader:
        if i == 0:
...
with open('1st_analysis.csv', 'w', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    for exist in exists:
